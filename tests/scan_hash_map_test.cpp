...
TEST(ScanHashMapTest, InsertScanMoveDetachAndErase) {
  ...
  for (std::uint64_t i = 0; i < kInitial; ++i) {
    ...
    ASSERT_CHECK(inserted.inserted && inserted.entry->value == i,
  ...
  for (std::uint64_t i = 0; i < 100; ++i) {
    ...
    ASSERT_CHECK(found != nullptr && found->value == i,
  ...
  for (std::uint64_t i = 0; i < kErasePopulation; i += 2) {
    ...
    ASSERT_CHECK(again.inserted, "reinsert after erase failed");
  }
