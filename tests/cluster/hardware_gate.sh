...
if [[ ${LAVIK_CLUSTER_HARDWARE_OPT_IN:-0} != 1 ]]; then
  echo "cluster hardware tests skipped: set LAVIK_CLUSTER_HARDWARE_OPT_IN=1"
  exit 77
fi

device=${LAVIK_CLUSTER_SCRATCH_DEVICE:-}
if [[ -z $device ]]; then
  echo "LAVIK_CLUSTER_SCRATCH_DEVICE is required after hardware opt-in" >&2
  exit 1
fi
