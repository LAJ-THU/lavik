...
Task<absl::StatusOr<HashResult>> StorageEngine::Impl::ExecuteHashLikeLocked(
  ...
  // through append's later allocation waits. Other keys need not wait for this
  // key's disk read or private decode/modify/encode work. Keep creation,
  // grouped/external values, EXEC and native candidate/loading on their
  // existing paths; ordinary online replay has the same command guards and
  // may use this path too. Snapshot only value metadata before suspension,
  // never an index entry pointer. GC is allowed to relocate this same logical
  // version while the state mutex is released.
  // Set multi-key mutation adapters always provide their durable TxShardWrites;
  ...
  KEYLANE_FAULT_INJECT(if (unlocked_compact_write &&
    ...
    const char* configured = std::getenv("KEYLANE_COMPACT_HASH_WRITE_PAUSE_MS");
  ...
  if (unlocked_compact_write) {
    ...
    const auto valid = ValidateCompactWriteSnapshot(
        store, partition, db_id, key, digest, location, observed_write);
