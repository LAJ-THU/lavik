  ...
  while (value_offset < value.size()) {
    ...
    BlockHeader header{
        ...
        .committed_bytes_ =
            static_cast<std::uint32_t>(kBlockHeaderBytes + payload_bytes),
        .record_count_ = 0,
        .max_lsn_ = next_lsn_.fetch_add(1, std::memory_order_relaxed),
        .header_sequence_ = 1,
        .checksum_ = 0,
        .layout_worker_count_ = worker_count_,
        .kind_ = BlockKind::kValueExtent,
        .reserved_ = {},
    ...
    std::memcpy(staging.data_ + kBlockHeaderBytes, payload.data(),
                payload.size());
...
Task<absl::Status> StorageEngine::Impl::WriteRecordLocked(
  ...
  RecordHeader record{
      ...
      .mutation_sequence_ = mutation_sequence,
      .relocation_sequence_ = relocation_sequence,
      .expire_at_ms_ = expire_at_ms,
      .lsn_ = lsn,
  ...
  if (!value.empty()) {
    std::memcpy(staging.data_ + record_offset + record_header_bytes,
