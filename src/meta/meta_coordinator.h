...
//     (MetaCommitEventSink, meta_state_machine.h). The sink is O(1) and never
...
// Opaque propose-timeout machinery (meta_coordinator.cpp); see
// MetaCoordinatorOptions::propose_timeout_ms_.
class MetaProposeTimer;
class MetaAuditProposalGate;

// ---------------------------------------------------------------------------
// Trust boundary: passkey + authenticated principal (see the file header).
// ---------------------------------------------------------------------------
...
class MetaPrincipalPasskey {
 private:
  ...
  friend class MetaCtlServer;            // authenticated ctl entry
  friend class MetaCoordinatorTestPeer;  // tests/meta_coordinator_test.cpp
};

// The authenticated identity of a privileged proposer, injected into the
// command's ActorContext by Propose. Value type; copyable inside trusted
// code, but only constructible through MetaPrincipalPasskey.
...
// MetaCommittedFacts (observation freshness queries, meta_observation_store.h)
...
// handles (defined in meta_coordinator.cpp); the shared_ptr keeps it alive
...
class MetaCoordinator {
 ...
 private:
  ...
  // opaque machinery lives in meta_coordinator.cpp and holds only weak
