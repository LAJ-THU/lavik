...
- `RecordLocation`: 72 bytes (previously 96).
- index entry header: 88 bytes (previously 152, before separately allocated key
  characters).

The allocator charge for an entry is the size class containing `88 + key
