...
| Meta control plane separates deterministic committed state, pure node projection, volatile observations, leader-scoped Data publishing, authenticated administration, stable cluster status and initial creation, and NuRaft integration | `include/keylane/meta/`, `src/meta/`, `app/keylane_meta.cpp`, `app/keylane_ctl.cpp` |
