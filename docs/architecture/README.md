...
| Meta control plane | NuRaft-backed committed metadata, leader-local observations and coordination, authenticated administration, membership identity, and WAL/snapshot recovery; linked only into `keylane-meta` | [Meta control plane](08-meta-control-plane.md) |
...
|---|---|
| The process composes the `keylane` data-plane executable and one main Keylane library around Celer, plus the separate meta-plane `keylane-meta` executable | `CMakeLists.txt`, `app/keylane.cpp`, `src/redis/server.cpp`, `src/meta/` |
...
| Meta control plane separates deterministic committed state, volatile observations, leader-only proposal coordination, authenticated administration, and NuRaft integration | `src/meta/meta_state_apply.*`, `src/meta/meta_observation_store.*`, `src/meta/meta_coordinator.*`, `src/meta/meta_ctl_server.*`, `src/meta/nuraft_*`, `app/keylane_meta.cpp`, `app/keylane_meta_ctl.cpp` |
