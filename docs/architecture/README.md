...
| Cluster data plane | Slot routing, finite authority admission, controlled-failover write pause, node-local failover activation and replica following, Meta discovery/session handling, and Redis Cluster compatibility | [Cluster data plane](06-cluster-data-plane.md) |
| Meta control plane | NuRaft-backed metadata, manifest-bootstrapped multi-member genesis, topology-owned single-Data-cluster lifecycle with atomic creation admission and per-Group failover transitions, Data-session publishing, cluster status, leader-owned workflow recovery, membership identity, and WAL/snapshots | [Meta control plane](08-meta-control-plane.md) |
