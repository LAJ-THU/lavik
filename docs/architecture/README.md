...
| Meta control plane | NuRaft-backed metadata, manifest-bootstrapped multi-member genesis, topology-owned single-Data-cluster lifecycle and atomic creation admission, Data-session publishing, cluster status, leader-owned workflow recovery, membership identity, and WAL/snapshots | [Meta control plane](08-meta-control-plane.md) |
...
| Cluster data plane has topology, finite authority, node-controller, Meta-client/session, and Redis gate boundaries | `include/keylane/cluster/`, `src/cluster/`, `src/redis/cluster_command.cpp`, `src/redis/command.cpp`, `src/redis/server.cpp` |
| Meta control plane separates deterministic committed state with a topology-owned cluster lifecycle, pure node projection, volatile observations, leader-scoped Data publishing, authenticated administration, atomic initial creation, stable cluster status, manifest-bootstrapped initial membership, and NuRaft integration | `include/keylane/meta/`, `src/meta/`, `app/keylane_meta.cpp`, `app/keylane_ctl.cpp` |
