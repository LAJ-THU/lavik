...
t5 SET conclude→出队→Poll→GET2 起跑;完后全零删条目
```
...
class Transaction {
  ...
  ShardCallback cb_; bool concluding_; uint8_t phase_;   // 回调=函数指针+void* ctx,避免 std::function 堆分配
...
- Transaction 公开:`InitKeys` / `Schedule()` / `Execute(cb, conclude)` / `Conclude()`。(RENAME 双 hop 是该 API 的验证命令,作可选扩展项。)
