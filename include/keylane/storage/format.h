...
inline constexpr std::size_t kRecordHeaderBytes = kDirectIoAlignment;
...
struct RecordHeader {
  ...
  std::uint32_t version = kStorageFormatVersion;
  std::uint16_t header_bytes = kRecordHeaderBytes;
...
constexpr std::size_t MaxKeyBytes() noexcept {
  return kRecordHeaderBytes - sizeof(RecordHeader);
}
...
bool DecodeRecordHeader(std::span<const std::byte, kRecordHeaderBytes> input,
